--
-- Cấu trúc bảng `caipiao_gscplus_bet_data`
-- Bản ghi cược chi tiết GSC+
--
...
CREATE TABLE `caipiao_gscplus_bet_data` (
  ...
  `wager_status` VARCHAR(50) NOT NULL COMMENT 'Trạng thái cược (đã thanh toán, đã hủy, v.v.)',
  `wager_type` VARCHAR(50) DEFAULT NULL COMMENT 'Loại cược',
  `currency` VARCHAR(10) NOT NULL DEFAULT 'CNY' COMMENT 'Loại tiền tệ',
  `settled_at` BIGINT(20) DEFAULT NULL COMMENT 'Thời gian thanh toán (mili giây)',
  `payload` JSON DEFAULT NULL COMMENT 'Dữ liệu gốc từ GSC+ (định dạng JSON)',
