--
-- Cấu trúc bảng `caipiao_gscplus_config`
-- Cấu hình API GSC+
--
...
CREATE TABLE `caipiao_gscplus_config` (
  ...
  `currency` VARCHAR(10) NOT NULL DEFAULT 'CNY' COMMENT 'Loại tiền tệ (CNY, USD, VND, v.v.)',
  `status` TINYINT(1) NOT NULL DEFAULT 1 COMMENT 'Trạng thái: 1=hoạt động, 0=không hoạt động',
...
--
-- Dữ liệu cấu hình mặc định cho GSC+
-- Lưu ý: Cập nhật operator_code, secret_key và URLs với giá trị thực tế
--
