...
class AuthController
{
    public function login(Request $request)
    {
        ...
        if (!$username || !$password) {
            return json(['code' => 400, 'message' => 'Tên người dùng và mật khẩu không được để trống', 'data' => null]);
        }
        ...
        if (!$user) {
            return json(['code' => ErrorCode::ACCOUNT_NOT_FOUND, 'message' => 'Tên người dùng hoặc mật khẩu sai', 'data' => null]);
        }
        ...
        if (!$passwordValid) {
            return json(['code' => ErrorCode::PASSWORD_ERROR, 'message' => 'Tên người dùng hoặc mật khẩu sai', 'data' => null]);
        }
        ...
        if ($user->islock == 1) {
            return json(['code' => ErrorCode::ACCOUNT_LOCKED, 'message' => 'Tài khoản đã bị khóa', 'data' => null]);
        }
    ...
    public function logout(Request $request)
    {
        ...
        return json(['code' => ErrorCode::SUCCESS, 'message' => 'Đăng xuấtThành công', 'data' => null]);
    }
    ...
    public function refresh(Request $request)
    {
        ...
        if (empty($refreshToken)) {
            return json(['code' => ErrorCode::TOKEN_INVALID, 'message' => 'Refresh Tokenkhông được để trống', 'data' => null]);
        }
        ...
        try {
            ...
            if (!$user) {
                return json(['code' => ErrorCode::USER_NOT_FOUND, 'message' => 'Người dùng không tồn tại', 'data' => null]);
            }
            
            if ($user->islock == 1) {
                return json(['code' => ErrorCode::ACCOUNT_LOCKED, 'message' => 'Tài khoảnđã bị khóa', 'data' => null]);
            }
            ...
            return json(['code' => ErrorCode::SUCCESS, 'message' => 'Làm mớiThành công', 'data' => $responseData]);
            
        } catch (\Exception $e) {
            return json(['code' => ErrorCode::SYSTEM_ERROR, 'message' => 'Làm mớiThất bại，请稍后重试', 'data' => null]);
        }
    }
    ...
    public function profile(Request $request)
    {
        ...
        if (!$userId) {
            return json(['code' => ErrorCode::UNAUTHORIZED, 'message' => '未Đăng nhậphoặcĐăng nhập已过期', 'data' => null]);
        }
    ...
    public function updateProfile(Request $request)
    {
        ...
        try {
            ...
            if (empty($updateData)) {
                return json(['code' => 400, 'message' => '没有需要更新的dữ liệu', 'data' => null]);
            }
            ...
            return json(['code' => 500, 'message' => '更新Thất bại：' . $e->getMessage(), 'data' => null]);
        }
    }
    ...
    public function changePassword(Request $request)
    {
        ...
        try {
            ...
            if (empty($oldPassword) || empty($newPassword) || empty($confirmPassword)) {
                return json(['code' => 1003, 'message' => '请填写完整的Mật khẩu信息', 'data' => null]);
            }
            
            if (strlen($newPassword) < 6 || strlen($newPassword) > 20) {
                return json(['code' => 1003, 'message' => '新Mật khẩu长度必须在6-20位之间', 'data' => null]);
            }
            
            if ($newPassword !== $confirmPassword) {
                return json(['code' => 1002, 'message' => '两次Mật khẩu不一致', 'data' => null]);
            }
            
            $user = Db::table('caipiao_member')->where('id', $userId)->first();
            
            if (!$user) {
                return json(['code' => 1005, 'message' => 'Người dùng không tồn tại', 'data' => null]);
            }
            
            $oldPasswordValid = password_verify($oldPassword, $user->password) || md5(sha1($oldPassword)) === $user->password;
            
            if (!$oldPasswordValid) {
                return json(['code' => 1001, 'message' => '原Mật khẩu sai', 'data' => null]);
            }
            
            $newPasswordHash = password_hash($newPassword, PASSWORD_BCRYPT);
            Db::table('caipiao_member')->where('id', $userId)->update(['password' => $newPasswordHash]);
            
            return json(['code' => 0, 'message' => 'Mật khẩuSửaThành công', 'data' => null]);
            
        } catch (\Exception $e) {
            return json(['code' => 500, 'message' => 'SửaThất bại：' . $e->getMessage(), 'data' => null]);
        }
    }
    ...
    public function setFundPassword(Request $request)
    {
        ...
        try {
            ...
            if (!preg_match('/^\d{6}$/', $password)) {
                return json(['code' => 1003, 'message' => '资金Mật khẩu必须是6位数字', 'data' => null]);
            }
            ...
            if ($hasSetFundPwd) {
                return json(['code' => 1007, 'message' => '已Cài đặt资金Mật khẩu，请使用Sửa接口', 'data' => null]);
            }
            
            $passwordMd5 = md5($password);
            Db::table('caipiao_member')->where('id', $userId)->update(['tradepassword' => $passwordMd5]);
            
            return json(['code' => 0, 'message' => '资金Mật khẩuCài đặtThành công', 'data' => null]);
            
        } catch (\Exception $e) {
            return json(['code' => 500, 'message' => 'Cài đặtThất bại：' . $e->getMessage(), 'data' => null]);
        }
    }
    ...
    public function changeFundPassword(Request $request)
    {
        ...
        try {
            ...
            if ($user->tradepassword !== $oldPasswordMd5) {
                return json(['code' => 1001, 'message' => '原资金Mật khẩu sai', 'data' => null]);
            }
            
            $newPasswordMd5 = md5($newPassword);
            Db::table('caipiao_member')->where('id', $userId)->update(['tradepassword' => $newPasswordMd5]);
            
            return json(['code' => 0, 'message' => '资金Mật khẩuSửaThành công', 'data' => null]);
